...
def nnPredict(w1,w2,data):
    ...
    for sam in range(n_samples):
        ...
        hid[0][:-1] = np.dot(inp,w1.T)
        hid[0][:-1] = sigmoid(hid[0][:-1])
        out_data = np.append(out_data,hid[0][:-1])
        ...
        if (initLabel):
            ...
            label = np.copy(temp)
            initLabel = True
    return label,out_data.reshape(n_samples,hid[0][:-1].size)
...
#def normalfunction(x, m, s):
#    #return np.divide(np.exp(-0.5*np.square(np.divide(np.subtract(x,m),s))),s*np.sqrt(np.pi*2))
#    return norm.pdf(x,m,s)
    
"""**************Neural-Bayes Script Starts here********************************"""
import time
...
predicted_label,train_hid_data = nnPredict(w1,w2,train_data)
#train_hid_dict = preprocess_cpt(train_hid_data,predicted_label,class_list)
train_hid_dict = preprocess_bayes_inp(train_hid_data,train_label,class_list)
savemat('train_hid_guassian.mat',train_hid_dict)
#cpt = construct_cpt(train_hid_dict,train_hid_data.shape[1])
norm_dist_mean, norm_dist_std = construct_guassian_dist(train_hid_dict, train_hid_data.shape[1], class_list)
#savemat('cpt_nn_bayes_hid.mat',cpt)
norm_dist = {}
norm_dist['mean'] = norm_dist_mean
norm_dist['std'] = norm_dist_std
savemat('norm_nnbayes_hid.mat', norm_dist)
#cpt = loadmat('cpt_nn_bayes_hid.mat')

#find the accuracy on Training Dataset
bayes_label = naive_bayes(norm_dist,train_hid_data)
...
predicted_label,valid_hid_data = nnPredict(w1,w2,validation_data)

#find the accuracy on Validation Dataset
bayes_label = naive_bayes(norm_dist,valid_hid_data)
...
predicted_label,test_hid_data = nnPredict(w1,w2,test_data)

#find the accuracy on Validation Dataset
bayes_label = naive_bayes(norm_dist,test_hid_data)
...
for i in range(10):
    ...
    num_digits[i] = test_label_digit[i].size
conf_matrix = conf_matrix.tolist()        
savemat('conf_matrix_norm_nnbayes_hid.mat',{'conf_matrix':conf_matrix,'conf_matrix_string':conf_matrix_string, 'num_digits': num_digits})
