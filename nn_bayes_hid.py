...
def preprocess_cpt(out,labels,n_class):
    ...
    out = out/np.max(out,1).reshape(out.shape[0],1)
    out = out*9.9
    out = out.astype('int')
...
def construct_cpt(out, n_nodes):
    ...
    #feature_matrix = np.ones(n_nodes,n_vals)
    cpt = {}
    for l in labels:
        out_data = out[str(l)]
...
def naive_bayes(cpt,test_data,labels):
    ...
    test_data = test_data/np.max(test_data,1).reshape(test_data.shape[0],1)
    test_data = test_data*9.9
    test_data = test_data.astype('int')
    test_labels = np.array([])
    for sam in range(test_data.shape[0]):
        prob = np.zeros(labels.size)
        data = test_data[sam]
        for l in labels:
...
train_hid_dict = preprocess_cpt(train_hid_data,train_label,class_list)
...
bayes_label = naive_bayes(cpt,train_hid_data,class_list)
...
bayes_label = naive_bayes(cpt,valid_hid_data,class_list)
...
#find the accuracy on Validation Dataset
bayes_label = naive_bayes(cpt,test_hid_data,class_list)
...
savemat('conf_matrix_nn_bayes_hid.mat',{'conf_matrix':conf_matrix,'conf_matrix_string':conf_matrix_string, 'num_digits': num_digits})
